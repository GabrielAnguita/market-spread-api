...
from spread.clients.buda import BudaAPIClient, BudaAPIEndpoint
from spread.clients.buda.types import Market
from spread.clients.buda.types import Ticker
from spread.clients.buda.types import OrderBook


class BudaAPIClientTestBase(TestCase):
    def setUp(self):
...
class TestGetTicker(BudaAPIClientTestBase):
    ...
    async def test_get_ticker(self):
        ...
        self.assertEqual(ticker.last_price.currency, "USD")
        self.assertEqual(ticker.min_ask.amount, 101.0)
        self.assertEqual(ticker.min_ask.currency, "USD")
        self.assertEqual(ticker.max_bid.amount, 99.0)
        self.assertEqual(ticker.max_bid.currency, "USD")
        self.assertEqual(ticker.volume.amount, 500.0)
        self.assertEqual(ticker.volume.currency, "BTC")
        self.assertEqual(ticker.price_variation_24h, 0.05)
        self.assertEqual(ticker.price_variation_7d, 0.1)
